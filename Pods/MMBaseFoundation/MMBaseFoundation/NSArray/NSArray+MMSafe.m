...
@implementation NSArray (MMSafe)

#pragma mark - safe hook
+ (void)load {
#ifdef DISTRIBUTION
    [self swizzleInstanceSelector:@selector(objectAtIndex:) withNewSelector:@selector(md_safeObjectAtIndex:)];
    [self swizzleClassSelector:@selector(arrayWithObjects:count:) withNewSelector:@selector(md_safeArrayWithObjects:count:)];
#endif
}

- (id)md_safeObjectAtIndex:(NSUInteger)index
{
    if (index >= self.count) {
        return nil;
    }
    return [self md_safeObjectAtIndex:index];
}

const NSUInteger loopLimit = 10;
+ (instancetype)md_safeArrayWithObjects:(const id [])objects count:(NSUInteger)cnt
{
    ...
    return [self md_safeArrayWithObjects:objects count:cnt];
}
...
+ (void)load {
#ifdef DISTRIBUTION
    [objc_getClass("__NSArrayM") swizzleInstanceSelector:@selector(addObject:) withNewSelector:@selector(md_safeAddObject:)];
    if (@available(iOS 15.0, *)){
        //在iOS 15 下 会引起内存持续增长，具体原因不明
    }else{
        [objc_getClass("__NSArrayM") swizzleInstanceSelector:@selector(objectAtIndex:) withNewSelector:@selector(md_safeObjectAtIndex:)];
    }
    [objc_getClass("__NSArrayM") swizzleInstanceSelector:@selector(insertObject:atIndex:) withNewSelector:@selector(md_safeInsertObject:atIndex:)];
    [objc_getClass("__NSArrayM") swizzleInstanceSelector:@selector(removeObjectAtIndex:) withNewSelector:@selector(md_safeRemoveObjectAtIndex:)];
#endif
}
...
- (void)md_safeInsertObject:(id)anObject atIndex:(NSUInteger)index {
    if (!anObject) {
        return;
    }
    
    [self md_safeInsertObject:anObject atIndex:index];
}

- (void)md_safeRemoveObjectAtIndex:(NSUInteger)index {
    if (index >= self.count) {
        return;
    }
    [self md_safeRemoveObjectAtIndex:index];
}
